...
bool HeightMap::RayTriangle(const XMVECTOR& vert0, const XMVECTOR& vert1, const XMVECTOR& vert2, const XMVECTOR& rayPos, const XMVECTOR& rayDir, XMVECTOR& colPos, XMVECTOR& colNormN, float& colDist)
{
	...
	colNormN = XMVector3Normalize(XMVector3Cross(vert0 - vert1, vert1 - vert2));

	/*if (abs(XMVectorGetY(colNormN)) > 0.99f)
	{
		return false;
	}*/
	...
	XMFLOAT3 ColDistDenominator;
	XMStoreFloat3(&ColDistDenominator, XMVector3Dot(colNormN, XMVector3Normalize(rayDir)));
	if (ColDistDenominator.x == 0)
	{
		return false;
	}
	// Step 4: Calculate the numerator of the COLDIST equation: -(D+(|COLNORM| dot RAYPOS))
	XMFLOAT3 ColDistNumerator;
	XMStoreFloat3(&ColDistNumerator, (D + XMVector3Dot(colNormN, rayPos)) * -1);

	// Step 5: Calculate COLDIST and "early out" again if COLDIST is behind RAYDIR
	colDist = ColDistNumerator.x / ColDistDenominator.x;
	if (colDist < 0)
	{
		return false;
	}
...
bool HeightMap::PointPlane(const XMVECTOR& vert0, const XMVECTOR& vert1, const XMVECTOR& vert2, const XMVECTOR& pointPos)
 {
	 ...
	 XMVECTOR colNormN = XMVector3Normalize(XMVector3Cross(vert0 - vert1, vert1 - vert2));
	 ...
	 if (XMVectorGetY(XMVector3Dot(colNormN, pointPos) + D) <= 0)
	 {
		 return false;
	 }

	 // Step 4: Return true! (in front of plane)
	 return true;
 }